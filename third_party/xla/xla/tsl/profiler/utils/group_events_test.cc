...
TEST(GroupTPUEventsTest, MergeHostStepsTest) {
  ...
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1", 1001, 8, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1", 1016, 98, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1", 1126, 48, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1", 1181, 23, {});
...
TEST(GroupTPUEventsTest, MergeOffloadedScSteps) {
  ...
  auto sc_op_line = sc_plane_builder.GetOrCreateLine(2);
  sc_op_line.SetName(kSparseCoreOpLineName);
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1a", 1110, 100, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_2a", 1610, 100, {});
...
TEST(GroupTPUEventsTest, GroupOffloadedSparseCoreModulesHostLoopTest) {
  ...
  auto tc_op_line = tc_plane_builder.GetOrCreateLine(2);
  tc_op_line.SetName(kXlaOpLineName);
  CreateXEvent(&tc_plane_builder, &tc_op_line, "offload_start", 1050, 100,
  ...
  auto sc_op_line = sc_plane_builder.GetOrCreateLine(2);
  sc_op_line.SetName(kSparseCoreOpLineName);
  CreateXEvent(&sc_plane_builder, &sc_op_line, "offloaded_start.copy", 1100, 10,
               {});
...
TEST(GroupTPUEventsTest, GroupOffloadedSparseCoreModulesDeviceLoopTest) {
  ...
  auto sc_op_line = sc_plane_builder.GetOrCreateLine(2);
  sc_op_line.SetName(kSparseCoreOpLineName);
  CreateXEvent(&sc_plane_builder, &sc_op_line, "offloaded_start.copy", 1100,
               100, {});
...
TEST(GroupTPUEventsTest, GroupSparseCoreEventsMissingOffloadMetadataTest) {
  tensorflow::profiler::XSpace space;
  // TPU TensorCore Plane (device_id 0)
  XPlane* tensorcore_plane = GetOrCreateTpuXPlane(&space, 0, "TPUv4", 0, 0);
  XPlaneBuilder tc_plane_builder(tensorcore_plane);
  ...
  tc_module_line.SetName(kXlaModuleLineName);
  CreateXEvent(&tc_plane_builder, &tc_module_line, "jit(1)", 1000, 1000,
               {{StatType::kRunId, int64_t{1}}});

  auto tc_op_line = tc_plane_builder.GetOrCreateLine(2);
  tc_op_line.SetName(kXlaOpLineName);
  CreateXEvent(&tc_plane_builder, &tc_op_line, "tc_op_1", 1010, 50, {});
  CreateXEvent(&tc_plane_builder, &tc_op_line, "tc_op_2", 1100, 50, {});
  ...
  sc_step_line.SetName(kSparseCoreStepLineName);
  CreateXEvent(&sc_plane_builder, &sc_step_line, "sc step 0", 1000, 499, {});
  CreateXEvent(&sc_plane_builder, &sc_step_line, "sc step 1", 1500, 499, {});

  auto sc_module_line = sc_plane_builder.GetOrCreateLine(1);
  sc_module_line.SetName(kSparseCoreModuleLineName);
  // These events are missing kConsumerId, so they won't link to TC.
  CreateXEvent(&sc_plane_builder, &sc_module_line, "offloaded(1)", 1000, 499,
               {});
  CreateXEvent(&sc_plane_builder, &sc_module_line, "offloaded(2)", 1500, 499,
               {});

  auto sc_op_line = sc_plane_builder.GetOrCreateLine(2);
  sc_op_line.SetName(kSparseCoreOpLineName);
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1a", 1010, 40, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_1b", 1100, 50, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_2a", 1510, 40, {});
  CreateXEvent(&sc_plane_builder, &sc_op_line, "sc_op_2b", 1600, 50, {});
  ...
  const GroupMetadataMap& group_metadata_map =
      event_forest.GetGroupMetadataMap();
  // We expect 3 groups: one for TensorCore (based on RunId), and two for
  // SparseCore (one for each offloaded module, as they lack ConsumerId).
  EXPECT_EQ(group_metadata_map.size(), 2);
  absl::flat_hash_set<int64_t> group_ids;

  // Check TensorCore events are grouped together.
  XPlaneVisitor tc_visitor = CreateTfXPlaneVisitor(tensorcore_plane);
  tc_visitor.ForEachLine([&](const XLineVisitor& line) {
    line.ForEachEvent([&](const XEventVisitor& event) {
      SCOPED_TRACE(absl::StrCat(tensorcore_plane->name(), ": ", line.Name(),
      ...
      std::optional<XStatVisitor> group_id_stat =
          event.GetStat(StatType::kGroupId);
      ASSERT_TRUE(group_id_stat.has_value());
      group_ids.insert(group_id_stat->IntValue());
    });
  });
  EXPECT_THAT(group_ids, ::testing::UnorderedElementsAre(0));
  group_ids.clear();

  // Check SparseCore events. Each module and its ops should form a group.
  XPlaneVisitor sc_visitor = CreateTfXPlaneVisitor(sparsecore_plane);
  sc_visitor.ForEachLine([&](const XLineVisitor& line) {
    line.ForEachEvent([&](const XEventVisitor& event) {
      SCOPED_TRACE(absl::StrCat(sparsecore_plane->name(), ": ", line.Name(),
  ...
  EXPECT_THAT(group_ids, ::testing::UnorderedElementsAre(0, 1));
}
