...
bool IsTPUStepLineEvent(const XEventVisitor& event) {
...
std::string EventNode::GetGroupName() const {
  ...
  } else if (!(IsImplicitRootEvent(visitor_) || IsTPUStepLineEvent(visitor_))) {
...
XLine* GetGroupingLineForTPU(XPlane* plane, const XPlaneVisitor& visitor) {
...
void EventForest::ConnectIntraThreadTPU(XPlane* plane, XPlaneVisitor* visitor,
  ...
  XLine* grouping_line = GetGroupingLineForTPU(plane, *visitor);
  ...
  } else if (sc_id.has_value()) {
    sparse_core_root_events_per_core_.emplace_back(std::move(parent_nodes));
  }
}
...
void EventForest::CreateEventGroups() {
  ...
  // Group TensorCore root events first just in case the SparseCore device step
  // events are nested under the TensorCore device step events.
  ...
  for (auto& core_root_events : sparse_core_root_events_per_core_) {
    uint64_t device_step_group_id = group_id;
    for (EventNode* root_event : core_root_events) {
      // If the SparseCore step wasn't grouped by the TensorCore, then group it
      // similarly here.
      if (RootNeedsGrouping(root_event)) {
        ProcessRootEvent(device_step_group_id++, root_event,
                         &group_metadata_map_);
