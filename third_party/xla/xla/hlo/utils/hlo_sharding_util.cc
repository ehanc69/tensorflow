...
void DevicesForShardingInternal(
  ...
  // TODO: In our case we might have Replicated sharding with mesh, so should we
  // check for those mesh devices ?
  // In current sharding format, replicated sharding is independent of devices.
  ...
  std::vector<int64_t> used_devices = sharding.UsedDevices();
  DCHECK(std::all_of(
